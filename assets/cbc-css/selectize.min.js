...
! function (root, factory) {
	...
	var cmp = function (a, b) {
		...
		is_array = Array.isArray || "undefined" != typeof $ && Array.isArray || function (object) {
	...
	return MicroEvent.mixin(Selectize), void 0 !== MicroPlugin ? MicroPlugin.mixin(Selectize) : (message = "Dependency MicroPlugin is missing", options = (options = {
		...
		function init_select($input, settings_element) {
			...
			function addOption($option, group) {
				...
				(value || settings.allowEmptyOption) && (optionsMap.hasOwnProperty(value) ? group && ((option = optionsMap[value][field_optgroup]) ? Array.isArray(option) ? option.push(group) : optionsMap[value][field_optgroup] = [option, group] : optionsMap[value][field_optgroup] = group) : ((option = readData($option) || {})[field_label] = option[field_label] || $option.text(), option[field_value] = option[field_value] || value, option[field_disabled] = option[field_disabled] || $option.prop("disabled"), option[field_optgroup] = option[field_optgroup] || group, optionsMap[value] = option, options.push(option), $option.is(":selected") && settings_element.items.push(value)))
			}
